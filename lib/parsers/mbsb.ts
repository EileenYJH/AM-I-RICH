...
export function parseMBSB(text: string): ParseResult | null {
  if (!/MBSB/i.test(text)) return null

  const balanceMatch = text.match(/(?:Available\s+Balance|Current\s+Balance|Balance)\s*\n?\s*(?:MYR|RM)\s*([\d,]+\.\d{2})/i)
  if (!balanceMatch) {
    const fallback = firstRM(text)
    if (!fallback) return null
    return { institution: 'MBSB Bank', accountName: 'Savings', balance: fallback, transactions: [] }
  }

  const balance = parseFloat(balanceMatch[1].replace(/,/g, ''))
  return { institution: 'MBSB Bank', accountName: 'Savings', balance, transactions: extractTransactions(text) }
}
