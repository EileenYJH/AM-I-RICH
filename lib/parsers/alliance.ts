...
export function parseAlliance(text: string): ParseResult | null {
  if (!/Alliance/i.test(text)) return null
  ...
  if (!balanceMatch) {
    ...
    return { institution: 'Alliance Bank', accountName: 'Savings', balance: fallback, transactions: [] }
  }

  const balance = parseFloat(balanceMatch[1].replace(/,/g, ''))
  return { institution: 'Alliance Bank', accountName: 'Savings', balance, transactions: extractTransactions(text) }
}
